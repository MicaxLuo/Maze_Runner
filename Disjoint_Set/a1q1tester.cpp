...
bool test6(std::string& error){
	...
	for(int i=0;rc && i<100;i+=2){ // 50 sets
		result = theSet.unionSets(i,i+1);
	}
	for(int i=0;rc && i<50;i+=2){
		theSet.unionSets(i*2,(i+1)*2); // 25 sets
	}
...
bool test7(std::string& error){
	...
	bool rc=true;
	for(int i=0;i<100;i++){
		theSet.makeSet(i); // 100
	}
	for(int i=0;rc && i<100;i+=2){
		result = theSet.unionSets(i,i+1); // 50
	}
	for(int i=0;rc && i<50;i+=2){
		theSet.unionSets(i*2,(i+1)*2); // 25
	}
	for(int i=0,j=99;rc && i<48;i+=4,j-=4){ // 13
